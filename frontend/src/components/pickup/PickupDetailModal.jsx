              ...
              <motion.dl variants={rowStagger} initial="hidden" animate="show" className="space-y-3 text-sm">
                ...
                {(pickup.contactName || pickup.user?.name) && (
                  <motion.div variants={rowItem} className="flex items-center justify-between">
                    ...
                    <dd className="text-ink">{pickup.contactName || pickup.user?.name}</dd>
                  </motion.div>
                )}
                {(pickup.contactPhone || pickup.user?.phone) && (
                  <motion.div variants={rowItem} className="flex items-center justify-between">
                    <dt className="text-inkFaint">Contact</dt>
                    <dd>
                      <a
                        href={`tel:${pickup.contactPhone || pickup.user?.phone}`}
                        className="flex items-center gap-1.5 text-ink font-mono font-semibold hover:text-rust transition-colors"
                      >
